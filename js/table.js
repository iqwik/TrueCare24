class Table {
    ...
    _renderUser(user, btnCall){
        ...
        let $hiddenName = $('<input/>', {
            type: "hidden",
            name: "name",
            value: user.name
        });
        let $hiddenPhone = $('<input/>', {
            type: "hidden",
            name: "phone",
        ...
        $hiddenName.appendTo($formCall);
        $hiddenPhone.appendTo($formCall);
        $btnCall.appendTo($formCall);
        $formCall.appendTo($divBtns);
        let $kebabMenu = $('<a/>', {
            class: "kebab-menu",
            href: "#"
        });
    ...
    _callTo(element, btnClass){
        element.click(e => {
            ...
            $form.find('.input-hidden._from').val('+12763257427');
            ...
            if(!element.hasClass('calling')){
                ...
                alert(`\nwe will call to #: ${element.data('user-phone')}...\n\nbut now we're calling to test #: +12763257427`);
                ...
                $.ajax({
                    ...
                    .done(data => {
                        console.log(`success! Incoming call... ${data}`);
                    })
